...
public class VectorQuantization {
    public static void Compress(String Path,int codeBookSize,int vectorHeight,int vectorWidth){
        int[][] originalImage=RWCompression.ReadImage(Path);
    ...
    private static void Quantize(Vector<Vector<Integer>>vectorOfBooks,Vector<Vector<Integer>>quantizedVector,int level){
        ...
        for (int i = 0; i <vectorOfBooks.size(); i++) {
            if(EcludianDistance(vectorOfBooks.get(i), average, 1)<EcludianDistance(vectorOfBooks.get(i), average, -1)){
